...
import click 
import numpy as np
#get 100 binsize metlevel from deepsignal-plant output
@click.command()
...
def level_size(geno_file,met_file,prefix):
    ...
    met_df_interval_group.query('size>=4').reindex(['chrom','kind','interval','met_level','size'],axis=1).drop_duplicates().to_csv(f'/public/home/lizw/task/pore_c/methylation/{prefix}_level_size_in_interval.csv')
